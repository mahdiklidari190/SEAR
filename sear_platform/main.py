...
Preserves all original CLI functionality while adding enterprise features.
"""
...
def display_header():
    """Display application header - preserved from original."""
    ascii_art = """
 ███████╗███████╗ █████╗ ██████╗ 
 ██╔════╝██╔════╝██╔══██╗██╔══██╗
 ███████╗█████╗  ███████║██████╔╝
...
async def process_single_page(
    ...
    """Process a single page through the full analysis pipeline."""
    filename = sanitize_filename(urlparse(url).path)

    try:
        console.print(f"[cyan]>> Fetching page...[/cyan]")
        fetch_result = await fetcher.fetch(url)

        console.print(f"[cyan]>> Extracting & Validating...[/cyan]")
        page_data = ContentExtractor.extract(fetch_result, url)

        console.print(f"[cyan]>> Extracting Keywords...[/cyan]")
        keywords = KeywordExtractor.extract(page_data)
        page_data.keywords = keywords

        console.print(f"[cyan]>> Finding Competitors...[/cyan]")
...
async def _fetch_robots_txt(fetcher: RobustFetcher, base_url: str) -> str:
    """Fetch robots.txt - preserved from original."""
...
def _serve_dashboard(directory: Path, port: int = 8765) -> None:
    """Launch a lightweight HTTP server and open the dashboard."""
    import os
    os.chdir(directory)
    ...
    try:
        ...
        # Port in use, try opening file directly
...
async def main_flow(input_url: str):
    """Main analysis workflow - preserved and extended."""
    settings = get_settings()
    fetcher = RobustFetcher(
    ...
    console.print("\n[bold cyan]Initializing analysis engine...[/bold cyan]")
    urls: list[str] = []

    try:
        result = await fetcher.fetch(input_url)
        text = result.content.decode("utf-8", errors="ignore")

        if "<sitemapindex" in text or "<sitemap>" in text or RE_LOC.search(text):
            console.print("[yellow][!] Sitemap detected. Crawling recursively...[/yellow]")
            seen = {input_url}
            urls = await SitemapParser.parse_recursive(fetcher, input_url, seen)
            if not urls:
                console.print("[yellow][!] No valid URLs in sitemap. Falling back to single URL.[/yellow]")
                urls = [input_url]
        else:
            urls = [input_url]

        if not urls:
            console.print("[red][ERROR] Could not extract any URLs.[/red]")
            await fetcher.close()
            return

    except Exception as e:
        console.print(f"[red][ERROR] Initial fetch failed: {e}. Falling back to homepage...[/red]")
    ...
    if len(urls) > max_pages:
        console.print(f"[yellow]Limiting to {max_pages} pages (found {len(urls)}).[/yellow]")
        urls = urls[:max_pages]

    console.print(Rule())
    console.print(f"[bold green]{len(urls)} page(s) queued for analysis.[/bold green]")
    console.print(Rule())
    ...
    if sc_integration.is_available:
        console.print("[dim]Fetching Search Console data...[/dim]")
        search_console_data = sc_integration.fetch_data()
    if an_integration.is_available:
        console.print("[dim]Fetching Analytics data...[/dim]")
        analytics_data = an_integration.fetch_data()
    ...
    # Anchor Text
    anchor_report = anchor_analyzer.analyze()

    # Generate site-level exports
...
def main():
    """CLI entry point - preserved from original."""
    check_dependencies()
    ...
    while True:
        ...
        input_url = console.input("\n[bold yellow]Enter Sitemap or Direct Webpage URL: [/bold yellow]").strip()
